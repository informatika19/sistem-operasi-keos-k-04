...
int copy_arr(char *arr, char *str, int isNullChar) {
  int i=0;
  while (str[i] != '\0') arr[i++] = str[i];
  if (isNullChar) arr[i] = '\0';
  return i;
}

// copy array 2 ke array 1 sebanyak length
void copy_arr_length(char *arr, char *str, int length, int isNullChar) {
  int i=0;
  while (i < length) {arr[i++] = str[i];}
  if (isNullChar) arr[i] = '\0';
}
